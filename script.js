...
const HISTORY_CARDS = [
  ...
  { title: 'Heritable vs. somatic edits.',        body: 'A somatic edit changes one body — the person who chose it. A heritable (germline) edit changes the egg, sperm, or embryo, so it carries into every cell of the resulting person and into any children they have. Somatic edits are revisable in principle and consented to by the patient; heritable edits are made for someone who does not yet exist and cannot, by construction, consent. The Oviedo Convention (Article 13) restricts modifications of the human genome to preventive, diagnostic, or therapeutic purposes and forbids changes to descendants — one of the clearest places this distinction is encoded in law.' }
];
