...
const HISTORY_CARDS = [
  ...
  { title: 'Genes ≠ destiny.', body: 'Twin studies put Big Five personality traits at roughly 40–50% heritable (Polderman et al., 2015, meta-analyzed ~17,800 traits). Most of the rest tracks non-shared environment — the unique experiences, peer groups, and accidents that even identical twins don\'t share. Shared family environment explains less of adult personality than people expect.',
    i18n: {
      zh: { title: '基因 ≠ 命运。', body: '双生子研究估计五大人格特质的遗传率大约为 40–50%(Polderman 等,2015,荟萃分析约 17,800 项性状)。其余大部分变异来自非共享环境——独特的经历、同伴圈,以及连同卵双胞胎都不曾共享的种种偶然。共享家庭环境对成年人格的解释力,比一般人想象的要小。' },
      ja: { title: '遺伝子 ≠ 運命。', body: '双生児研究は、ビッグファイブ性格特性の遺伝率をおよそ40〜50%と推定している(Polderman ら 2015、約17,800 形質のメタ解析)。残りの大部分は非共有環境に由来する——一卵性双生児でさえ共有しない、独自の経験、仲間集団、偶然などである。家庭で共有される環境が成人期の性格に与える影響は、世間が想像するよりも小さい。' },
      ko: { title: '유전자 ≠ 운명.', body: '쌍둥이 연구는 빅 파이브 성격 특성의 유전율을 약 40~50%로 추정한다(Polderman 외, 2015 — 약 17,800개 특성에 대한 메타분석). 나머지 대부분은 비공유 환경에서 기인한다 — 같은 부모 밑에서 자란 일란성 쌍둥이조차 공유하지 않는 고유한 경험과 또래, 그리고 우연들이다. 공유된 가정 환경이 성인기 성격을 설명하는 비중은 사람들이 짐작하는 것보다 작다.' },
      tr: { title: 'Genler ≠ kader.', body: 'İkiz çalışmaları, Beş Büyük kişilik özelliklerinin kalıtsallığını yaklaşık %40-50 olarak hesaplar (Polderman vd., 2015 — yaklaşık 17.800 özelliğe ilişkin meta-analiz). Kalanın çoğu paylaşılmayan çevreden gelir — tek yumurta ikizlerinin bile paylaşmadığı kişisel deneyimler, akran grupları ve rastlantılar. Paylaşılan aile çevresi, yetişkin kişiliğini insanların sandığından daha az açıklar.' }
    } },
  ...
  { title: 'Heritable vs. somatic edits.', body: 'A somatic edit changes one body. A heritable (germline) edit changes an egg or embryo — so it carries into every cell of the resulting person, and into their children. The Oviedo Convention (a 1997 Council of Europe biomedicine treaty, ratified by 29 states) restricts genome edits to preventive, diagnostic, or therapeutic purposes (Article 13); that purpose-test is where the legal line falls in much of Europe.',
    i18n: {
      ...
      tr: { title: 'Kalıtsal düzenlemeler ile somatik düzenlemeler.', body: 'Somatik bir düzenleme tek bir bedeni değiştirir. Kalıtsal (germ hattı) düzenleme bir yumurta ya da embriyoyu değiştirir — bu yüzden ortaya çıkan kişinin her hücresine ve onun çocuklarına taşınır. 1997 tarihli Avrupa Konseyi Oviedo Sözleşmesi (29 devletin onayladığı bir biyotıp anlaşması), genom düzenlemelerini önleyici, tanısal veya tedavi edici amaçlarla sınırlandırır (Madde 13); Avrupa\'nın büyük bölümünde hukuki sınır işte bu "amaç testidir".' }
    } },
